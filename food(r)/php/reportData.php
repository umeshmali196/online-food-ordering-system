...
if(isset($_GET['charts']))
{
  ...
  if($_GET['tables'] == "Today"){
    $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                      ...
                                      WHERE r_id=".$_SESSION['resid']." AND DATE(order_date)=CURDATE()";
    // echo $sql;
  }
  if($_GET['tables'] == "Yesterday"){
      $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                        ...
                                        INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id 
                                        INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                        WHERE r_id=".$_SESSION['resid']." AND DATE(order_date)=SUBDATE(CURDATE(),INTERVAL 1 DAY)";
  }
  if($_GET['tables'] == "Last7days"){
    $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                      ...
                                      INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id
                                      INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                      WHERE r_id=".$_SESSION['resid']." AND order_date BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW()";
    echo $sql;
  }
  if($_GET['tables'] == "Last15days"){
    $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                      ...
                                      INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id
                                      INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                      WHERE r_id=".$_SESSION['resid']." AND order_date BETWEEN DATE_SUB(NOW(), INTERVAL 15 DAY) AND NOW()";
  }
  if($_GET['tables'] == "Lastweek"){
    ...
    $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                      ...
                                      INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id
                                      INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                      WHERE r_id=".$_SESSION['resid']." AND WEEK(order_date,7)=$date";
  }
  if($_GET['tables'] == "Lastmonth"){
      ...
      $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                        ...
                                        INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id
                                        INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                        WHERE r_id=".$_SESSION['resid']." AND MONTH(order_date)=$month";
  }
  if($_GET['tables'] == "FromTo"){
    ...
    $sql = "SELECT * FROM `tbl_order` INNER JOIN `payment` ON tbl_order.o_id=payment.o_id 
                                        ...
                                        INNER JOIN `orderd_item_details` ON orderd_item_details.o_id=tbl_order.o_id
                                        INNER JOIN `user_address` ON user_address.a_id=tbl_order.a_id 
                                      WHERE r_id=".$_SESSION['resid']." AND DATE(order_date) >= '$from' AND DATE(order_date) <= '$to'";
                                      // echo $sql;
  }
  $result = mysqli_query($conn, $sql);
  if(mysqli_num_rows($result) > 0){
