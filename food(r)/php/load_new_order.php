...
$sql = "SELECT * FROM tbl_order where order_status='new' and r_id=$resid ORDER BY o_id ASC LIMIT $start_from, $limit";
...
<div class="card ml-2 mt-2 box-shadow" style="width: 100%;">
    <div class="card-body p-0 bg-blue">
        <span class="font-weight-bold float-left m-1 text-light">New Order</span>
        <div class="font-weight-bold float-right m-1 text-light">
    ...
    <div class="card-footer p-2 bg-transparent">
        <button class="btnshow float-right rounded-circle  border-0 "
            onclick="showorder(<?php echo $row['o_id']; ?>);"><i class="fas fa-angle-double-right"></i></button>
    </div>
</div>
<?php
    }
