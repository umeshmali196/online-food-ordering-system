...
$sql = "SELECT * FROM tbl_order where order_status='ontheway' and o_id=$oid";  
...
<div class="card ml-2 box-shadow" style="width: 100%" id="cardetail">
  <div class="card-header p-0 pl-1 bg-transparent border-bottom-0">
    <span class="badge bg-blue px-4 py-2 text-light rounded-0 font-weight-bolder text-uppercase">on the way</span>
  ...
  $sql2 = "SELECT * FROM orderd_item_details where o_id=$oid";  
    ...
    <div class="card-footer p-1 bg-transparent mt-2 text-center">
      <button class="py-1 px-2  mt-3 mb-2 border shadow-lg rounded  bg-blue text-white font-weight-bold border-0 "
       onclick="orderdeliverd(<?php echo $row['o_id']; ?>)">
