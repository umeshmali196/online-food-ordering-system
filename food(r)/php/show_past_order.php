...
$sql = "SELECT * FROM tbl_order where order_status='deliverd' and o_id=$oid";  
$result = mysqli_query($conn, $sql);   
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {  
        ?>
<div class="card mt-3 ml-3 box-shadow" style="width: 100%" id="cardetail">
  <div class="card-header p-0 pl-1 bg-transparent border-bottom-0">
    <span class="badge badge-pill bg-blue text-white  px-4 py-2">Delivered</span>
    <div class="text-muted font-weight-bold float-right m-1 small">ON <?php echo $row['order_date'] ?></div>
  </div>
